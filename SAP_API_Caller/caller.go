...
import (
	...
	sap_api_output_formatter "sap-api-integrations-sales-contract-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callSalesContractSrvAPIRequirementHeader("A_SalesContractHeader", header)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callSalesContractSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	url := fmt.Sprintf("A_SalesContractHeader('%s')/to_Item", item.SalesContract)
	outputDataItem, err := c.callSalesContractSrvAPIRequirementItem(url, item)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callSalesContractSrvAPIRequirementItem(api string, item *requests.Item) (*sap_api_output_formatter.Item, error) {
